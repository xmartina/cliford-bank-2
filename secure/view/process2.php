...
function initiateTransferFunds()
{
	...
	if(dbNumRows($results) > 0) {
		...
		if($status == 'INACTIVE') {
			$msg = 'your Transfer has been placed on hold by our customer services. you need to contact us for verification of the account 
			 .';
			header('Location: index.php?v=local-transfer&msg=' . urlencode($msg));
			exit();
		}
	}
	...
	$_SESSION['otp_token'] = $token;
    	...
    	$owneremail="sophyinfotech@gmail.com";/* your bulk sms account email */
            			...
            			$message="Your PREMIER One Time Password OTP is $token, Valid for 10 mins to complete your transactions
            						"; /* message to be sent*/
	...
	header('Location: index.php?v=Token2');
	exit();
}
...
function transferFunds() 
{
	...
	if($s_token == $token) {
		...
		header('Location: index.php?v=local-transfer&msg=' . urlencode('Transaction Authorization Code in not valid.'));
		exit();
	}
	...
	if($toption != "LT") {
		...
		$d_total 	= ($s_bal - $amt);
		if($s_bal < $amt) {
			header('Location: index.php?v=local-transfer&msg=' . urlencode('You do not have enough balance to proceed with this transfer.'));
			exit();	
		}
	...
	$comments = sprintf("Local");
		$sql = "INSERT INTO tbl_transaction (tx_no, tx_type, amount, date, description, r_bank, r_email, r_accno, swift, to_accno, status, tdate, currency, comments) 
				VALUES ('$tx_no', 'debit', $amt, NOW(), '$description', '$r_bank', '$r_email', '$r_accno', '$swift', '$sacc_no', 'SUCCESS', '$date_of', '$currency', '$comments')";
		dbQuery($sql);
...
<table width='100%' border='0' cellspacing='0' cellpadding='0' align='center'>
                        ...
                        <table width='100%' border='0' cellspacing='0' cellpadding='0' align='center' class='agile1 scale'>
                           ...
                           <tr ><td colspan='2' class='wls-5h' style='color:white; background-color:brown; text-align:center;'><h1><strong>Avenza Prime Bank </strong></h1></td></tr> 
                            ...
                            <tr>
								...
								<strong>LOCAL TRANSACTION NOTIFICATION</strong></td>
                            </tr>
                           ...
                           <tr>
                                <td class='w3l-p2' colspan='2' style='font-family: Candara, sans-serif; color:black; font-size: 18px; line-height: 28px;' class='scale-center-both'>
                                   The following local transactions has occured on your account, see below for transaction details .
                                </td>
                            </tr>
                            <tr><td height='12' colspan='2' style='font-size: 1px;'><hr></td></tr>
                            
                                      
                            <tr><td height='12' align='left' style='font-size: 14px; font-weight:700; color:black;'>TRANSACTION TYPE:</td>  <td style='font-size: 12px; font-weight:700; color:black;'> DEBIT</td></tr>
                             <tr><td height='12' align='left' style='font-size: 14px; font-weight:700; color:black;'>AMOUNT TRANSFERED: </td>  <td style='font-size: 12px; font-weight:700; color:black;'> $currency $amt</td></tr>
                             <tr><td height='12' align='left' style='font-size: 14px; font-weight:700; color:black;'>DATE OF TRANSACTION: </td>  <td style='font-size: 12px; font-weight:700; color:black;'>$date_of</td></tr>
                             <tr><td height='12' align='left' style='font-size: 14px; font-weight:700; color:black;'>DESCRIPTION:</td>  <td style='font-size: 12px; font-weight:700; color:black;'>$description</td></tr>
                             <tr><td height='12' align='left' style='font-size: 14px; font-weight:700; color:black;'>TRANSACTION REASON:</td>  <td style='font-size: 12px; font-weight:700; color:black;'> Local Transaction</td></tr>
                             <tr><td height='12' align='left' style='font-size: 14px; font-weight:700; color:black;'>REFERENCE NUMBER: </td>  <td style='font-size: 12px; font-weight:700; color:black;'> $tx_no</td></tr>
                            <tr><td height='12' align='left' style='font-size: 14px; font-weight:700; color:black;'>STATUS </td>  <td style='font-size: 12px; font-weight:700; color:black;'> SUCCESS</td></tr>
             
    	                    
    	                    <tr><td height='14' style='font-size: 14px; color:black;'><strong>AVAILABLE BALANCE: </td>  <td style='font-size: 14px; font-weight:700; color:black;'>  $currency $d_total</strong></td></tr>
    	          
    	        
                             <tr><td class='wls-5h' height='60' style='font-size: 14px; color:green;'> Warm Regards,</td></tr> 
	                        
	                         <tr><td class='wls-5h' height='30' style='font-size: 16px; color:orange;'><strong>Avenza Prime Bank </strong></td></tr>
	...
	//	funds_transfer_mail($amt, $sacc_no);
		...
		header('Location: index.php?v=LocalFund');
		exit();
	}
	...
	if (dbNumRows($result) == 1) {
		...
		$sql = "INSERT INTO tbl_transaction (tx_no, tx_type, amount, date, description, r_bank, r_email, r_accno, swift, to_accno, status, tdate, comments) 
				VALUES ('$tx_no', 'credit', $amt, NOW(), '$description', '$r_bank', '$r_email', '$r_accno', '$swift',  '$to_accno', 'SUCCESS', '$date_of', '$comments')";
		dbQuery($sql);
	...
	//email details...
		...
		header('Location: index.php?v=local-transfer&success=' . urlencode('Fund transfer successful.'));	
		exit();
	}
	else {
